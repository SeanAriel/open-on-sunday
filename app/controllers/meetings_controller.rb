class MeetingsController < ApplicationController
  ...
  before_action :set_meeting, only: [:edit, :destroy, :update, :confirm]
    ...
    def create
      ...
      @meeting.offer = @offer
      @meeting.save!
      @mess = Messtouser.new(title: 'Meeting Changed', content:" #{current_user.first_name} has made a new meeting with you", user_id: @meeting.offer.user_id)
         @mess.save!
    ...
    def update
      if @meeting.status == "Requested to Issuer"
        if @meeting.save!(meeting_params)
           ...
           @mess = Messtouser.new(title: 'Meeting Changed', content:" #{current_user.first_name} changed his date for the meeting #{@meeting.availability}", user_id: @meeting.offer.user_id)
           @mess.save!
           redirect_to dashboard_offers_path
        else
          render :edit
